...
function parseText(contents: string): Reviewable[] {
  ...
  return reviewables.map(([ fact, i ]: [ Fact, number ]) => (
                             { fact, header : (closestButNotOver(headers, i, (a, b) => a[1] - b) || [ "" ])[0] }));
}
...
function elapsedHours(d: Date, dnow?: Date){ return (((dnow || new Date()) as any) - (d as any)) / 3600e3 as number };
function reviewableToReview(reviewable: Reviewable, logs: QuizResult[]): Review {
  let relevantLogs = logs.filter(log => log.review.reviewable.fact.furigana
          && (ruby.furiganaStringToBoth(log.review.reviewable.fact.furigana)
                 === ruby.furiganaStringToBoth(reviewable.fact.furigana)));
...
function printReviewable(reviewable: Reviewable) {
  console.log(`${
                 ruby.furiganaStringToPlain(reviewable.fact.furigana)
...
function presentQuiz(review: Review, reviewables: Reviewable[]): QuizMetadata {
  ...
  if (review.subreview === 'kanji') {
    console.log(
        `Which of the following is the kanji for: ${ruby.furiganaStringToReading(review.reviewable.fact.furigana)}`);
...
function gradeAndDisplay(
    result: string, quiz: QuizMetadata, review: Review, reviewables: Reviewable[]): QuizResult|null {
  let time = new Date();
  ...
  if (result.indexOf('?') >= 0) {
    ...
    return {
      ...
      passive : false,
      time,
      memory : previousMemoryModel || DEFAULT_MEMORY_MODEL
    };
  }
  if (review.subreview === 'kanji') {
    ...
    if (selected
        && ruby.furiganaStringToPlain(selected.fact.furigana)
            === ruby.furiganaStringToPlain(review.reviewable.fact.furigana)) {
      ...
      return {
        ...
        passive : false,
        time,
        memory : previousTime ? ebisu.updateRecall(previousMemoryModel, true, elapsedHours(previousTime, time))
    ...
    if (selected) {
      ...
      return {
        ...
        passive : false,
        time,
        memory : previousTime ? ebisu.updateRecall(previousMemoryModel, false, elapsedHours(previousTime, time))
                              : DEFAULT_MEMORY_MODEL
      };
    }
    // Bad entry
...
if (require.main === module) {
  (async function() {
    ...
    if (reviews.length > 0) {
      ...
      if (toLog) { logReview(toLog, thisUser, 'l1'); }
    }
  })();
}
