...
function slurp(name: string): Promise<string> { return util.promisify(fs.readFile)(name, 'utf8'); }
function flatten1<T>(arr: T[][]): T[] { return arr.reduce((prev, curr) => prev.concat(curr)); }
if (require.main === module) {
  (async function() {
    ...
    if (Math.max(...reviews.map(x => x.length)) > 0) {
      ...
      let [pickedForReview, moduleIdx] = pickedForReviews.reduce(
          (prev, curr) => curr[0].recallProbability > prev[0].recallProbability ? prev : curr);
      ...
      let toLog = (modules[moduleIdx].gradeAndDisplay as any)(
          enteredText, quizDetails, pickedForReview, reviewables[moduleIdx]);
